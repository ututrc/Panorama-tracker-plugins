...
namespace HelpFunctions{
	...
	void filterRotationPoints(const std::vector<cv::Point2d> &vec1, const std::vector<cv::Point2d> &vec2, std::vector<cv::Point2d> &vec1Filtered, std::vector<cv::Point2d> &vec2Filtered)
	{
		...
		//Calculate avg distances in x- and y- axis
	...
	float slidingAverage(const std::list<float> &values)
	{
		float avg = 0;
		//Calculate the sliding average where older values have small factor
		std::vector<float> basic_avg(values.begin(), values.end());

		return calculateAverage(basic_avg);
	}
}