...
void PanoramaMap::LoopClose(float minPx, float maxPx, Size imgSize, Mat minImg, Mat maxImg)
{
	...
	ORB orb = ORB(200);
	orb.detect(minImg, kps1);
	orb.detect(maxImg, kps2);
	orb.compute(minImg, kps1, d1);
	orb.compute(maxImg, kps2, d2);
	...
	else
	{
		...
		// TODO: A smart solution, i.e. try matching to another location
		min_x_jump_ = minPx;
		max_x_jump_ = maxPx;
		x_mov = 0;
	}
