...
Mat ImageWarper::warpImageCylindrical(Mat image, float yaw, float pitch, float roll, Mat& result)
{
	...
	//Cut off 10 pixel sliced from each side to compensate for the undistortion
	//Mat sliced(undistorted.rows - 20, undistorted.cols - 20, undistorted.type());
	//Rect slice_rect(10, 10, sliced.cols, sliced.rows);
	//undistorted(slice_rect).copyTo(sliced);
	//Test by not using warp, instead just use the middle of the image
	//int sliced_pixels = 200;
	//Mat sliced(image.rows - sliced_pixels, image.cols - sliced_pixels, image.type());
	//Rect slice_rect(sliced_pixels / 2, sliced_pixels / 2, sliced.cols, sliced.rows);
	//image(slice_rect).copyTo(sliced);
	//result = sliced;
